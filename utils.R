...
#ALTRE FUNZIONI PER TIP (safe sumlog; unconstrain, funzioni per standard error)
...
fint=function(x,eta,theta,lower,upper) {
  ...
  eta*cv^(eta-1)*exp(theta*eta*cv^(eta-1))*sqrt(1-x^2)*(upper-lower)/2}
...
lambda.integral=function(alpha,beta,eta,theta,gamm,mu,XX,individual.hist,TT){
  ...
  if(is.na(somma.sep)) somma.sep=0 #aggiunta per linyip
  #parte self-correcting
  fuori=exp((XX%*%gamm)+mu)  
  tt2=c(0,individual.hist,TT)
  vett.int=(sapply(1:(length(tt2)-1), function(j) sum(fint(dd$nodes,eta,theta,tt2[j],tt2[j+1])*dd$weights))) 
...
lik.all=function(alpha.v,beta.v,eta,theta.v,gamm,mu.v,XX,obs.data,TT,WW,pi.C){
  sum(unlist(lapply(1:length(obs.data), 
                    function(j) -lik.unit.compl(alpha.v,beta.v,eta,
...
#Mi sono assicurata che a seconda di o, te, b funzioni tutto
